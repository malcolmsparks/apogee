...
(def ^:export default-chart-props {:x 50, :y 50,
...
(defn ^:export chart-background [{:keys [height width background-color
...
(defn ^:export x-grid [{:keys [height width
		      xmin xmax
		      grid-lines
...
(defn ^:export y-grid [{:keys [height width
		      ymin ymax
		      grid-lines
		      major-grid-color minor-grid-color
		      major-grid-width minor-grid-width]}]
...
(defn ^:export x-axis [{:keys [height width
		      xmin xmax
		      grid-lines
...
(defn ^:export y-axis [{:keys [height width
		      ymin ymax
		      grid-lines
		      axis-font-family axis-font-size
		      axis-number-format]}]
...
(defn ^:export point-label [{:keys [label-font-family
			   label-font-size
			   label-number-format]}
			    x* y* x y r & options]
...
(defn ^:export add-point [chart x y r & options]
...
(defn ^:export points->xy [points]
  (reduce (fn [[x y] [p1 p2]] [(conj x p1) (conj y p2)])
	  [[] []] points))

(defn ^:export xy->points [[x y]]
  (map (fn [p1 p2] [p1 p2]) x y))

(defn ^:export add-points [chart data & {:keys [size sizes colors transpose-data? fill]}]
